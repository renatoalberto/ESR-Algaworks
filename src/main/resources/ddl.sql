...
create table restaurante_formas_pagamento (restaurante_id bigint not null, forma_pagamento_id bigint not null) engine=InnoDB;
create table tab_cozinhas (id bigint not null auto_increment, nom_cozinha varchar(30) not null, primary key (id)) engine=InnoDB;
...
alter table restaurante add constraint FK1ypigtneik5hgnuo6824uq5y6 foreign key (cozinha_id) references tab_cozinhas (id);
alter table restaurante add constraint FKbc0tm7hnvc96d8e7e2ulb05yw foreign key (endereco_cidade_id) references cidade (id);
alter table restaurante_formas_pagamento add constraint FKg99fjbqkawklm5w9hul267oot foreign key (forma_pagamento_id) references forma_pagamento (id);
